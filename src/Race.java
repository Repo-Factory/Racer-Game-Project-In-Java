...
public class Race extends JFrame {
	...
	private void prepareToRace() {
		...
		while (input != 's' && input != 'S') {
			...
			case 'F':
			case 'f':
				Frog frog = new Frog("frog", START_LINE,yPos);
				racerList.add(frog);
				yPos = yPos + RACER_SPACE;
				break;
			case 'L':
			case'l':
				Lion lion = new Lion("lion", START_LINE,yPos);
				racerList.add(lion);
				yPos = yPos + RACER_SPACE;
				break;
			default: 
				JOptionPane.showMessageDialog(this, "Error: Not Valid Input for Racer");
			}
	...
	private char getRacer() {
		String input = JOptionPane.showInputDialog(this, "Enter a racer:"
				+ "\nt for Tortoise, h for hare, f for frog, l for lion," + "\nor s to start the race");
		if (input == null) {
